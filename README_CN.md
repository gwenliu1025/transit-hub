...
生产部署文件位于 `deploy/` 目录。默认从此 Fork 的源码构建镜像，不从上游拉取旧版本。经营表现按北京时间统计本周（周一至今）和本月（1 日至今），不使用滚动 7/30 天。正式产物见 [Releases](https://github.com/gwenliu1025/transit-hub/releases)。
...
# - 本地构建的镜像标签（默认 transit-hub:v0.1.16）
